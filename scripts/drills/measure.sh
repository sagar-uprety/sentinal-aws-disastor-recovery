...
disaster_ts="$(current_event_ts disaster_declared)"
outage_ts="$(current_event_ts outage_confirmed)"
invoked_ts="$(current_event_ts failover_invoked)"
promoted_ts="$(current_event_ts replica_promoted)"
stable_ts="$(current_event_ts secondary_service_stable)"
healthy_ts="$(current_event_ts secondary_targets_healthy)"
link_verified_ts="$(current_event_ts pre_outage_link_verified_in_secondary)"
write_ts="$(current_event_ts secondary_write_verified)"
switch_requested_ts="$(current_event_ts traffic_switch_requested_secondary)"
verified_ts="$(current_event_ts traffic_verified_secondary)"
primary_link_created_at="$(current_event_ts primary_link_created_at)"
replica_lag="$(current_event_ts replica_lag_seconds)"
replica_lag_timestamp="$(current_event_ts replica_lag_timestamp)"
