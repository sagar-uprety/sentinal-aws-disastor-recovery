...
readonly DR_SSM_ARN_PREFIX="arn:aws:ssm:${DR_REGION}:"
readonly DR_ECS_ALARM_NAME="${DR_RESOURCE_NAME}-ecs-running-tasks"
readonly DR_ALB_ALARM_NAME="${DR_RESOURCE_NAME}-alb-healthy-hosts"
...
determine_promotion_state() {
  ...
  if [[ "$replica_source" != "$PROD_RESOURCE_NAME" && "$replica_source" != *":db:${PROD_RESOURCE_NAME}" ]]; then
    echo "error: DR replica source is $replica_source, expected $PROD_RESOURCE_NAME" >&2
    exit 1
  fi
...
# requires resting DR compute before promotion starts.
...
# rejects mutable image tags because DR must run the exact tested prod artifact.
...
# resolves the regional database password before prod replication is broken.
password_parameter_arn="$(jq -r '.containerDefinitions[0].secrets[] | select(.name == "DB_PASSWORD") | .valueFrom' "$task_definition_json")"
if [[ "$password_parameter_arn" != "$DR_SSM_ARN_PREFIX"* ]]; then
  echo "error: DR task definition does not reference a $DR_REGION SSM password parameter" >&2
  exit 1
fi
...
# resolves the regional write token before prod replication is broken.
...
if [[ "$link_token_parameter_arn" != "$DR_SSM_ARN_PREFIX"* ]]; then
  echo "error: DR task definition does not reference a $DR_REGION link-token parameter" >&2
  exit 1
fi
aws ssm get-parameter \
  --region "$DR_REGION" \
...
# starts DR compute with the promoted writer endpoint and waits for stability.
...
# requires two-AZ task placement before DR can receive traffic.
...
# preserves the existing SNS destination when reactivating DR alarms.
...
# activates task-count alerting for serving DR compute.
...
# requires two healthy DR targets before application verification.
healthy_count=0
for _ in {1..60}; do
...
missing_slugs="$(require_all_short_links_direct "$WORKLOAD_HOST" "$dr_alb_dns" "$primary_slugs_before_outage")" || {
  echo "error: DR is missing pre-outage links: $missing_slugs; traffic must not be switched" >&2
  exit 1
}
# records wall-clock DR verification because replicated row timestamps cannot measure data-loss time.
log_event "pre_outage_link_verified_in_dr"

# proves promoted DR accepts new writes instead of only serving replicated reads.
...
Next operator-gated step:
  CONFIRM_TRAFFIC_SWITCH=DR scripts/drills/switch-traffic.sh dr
EOF
