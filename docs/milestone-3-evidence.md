...
**Outstanding:** the SNS email subscription is `PendingConfirmation`. The first confirmation email's link expired unused (status showed `"Deleted"`); the subscription was replaced via `terraform apply -replace` to resend it. The project owner must click the confirmation link in their inbox before alerts actually deliver to email — the alarm/EventBridge/SNS-publish mechanics are all confirmed working independently of that click.
...
- App logs: `"healthz: db ping failed", "error":"driver: bad connection"` then `"error":"pq: the database system is starting up (57P03)"` — clean structured errors, no DB password or connection string in the log lines.
...
- AWS Budgets: "100$ limit" (annual) at $0, "My Monthly Cost Budget" ($30/month) at $0.84 — both pre-date today's rebuild; Cost Explorer/Budget actuals lag roughly a day.
...
- `app.yml` not yet exercised by a real GitHub Actions run (needs PR merge to `main`).
- SNS email subscription needs the project owner to click the confirmation link.
