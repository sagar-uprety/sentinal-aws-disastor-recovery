...
IAM OpenID Connect role that lets `workload.yml` deploy immutable URL-shortener images without long-lived AWS keys.

## Design Intent

GitHub Actions exchanges its OIDC token for temporary credentials. The trust policy requires the exact repository and protected `production` environment. Policy scope covers one replicated ECR repository, reviewed regional task roles, task-definition revisions, and only the prod and DR workload services.
...
| <a name="input_ecr_repository_arn"></a> [ecr\_repository\_arn](#input\_ecr\_repository\_arn) | ARN of the ECR repository workload.yml pushes images to. | `string` | n/a | yes |
| <a name="input_ecs_cluster_arn"></a> [ecs\_cluster\_arn](#input\_ecs\_cluster\_arn) | ARN of the ECS cluster workload.yml deploys to. | `string` | n/a | yes |
| <a name="input_ecs_service_arn"></a> [ecs\_service\_arn](#input\_ecs\_service\_arn) | ARN of the ECS service workload.yml updates. | `string` | n/a | yes |
| <a name="input_ecs_task_execution_role_arn"></a> [ecs\_task\_execution\_role\_arn](#input\_ecs\_task\_execution\_role\_arn) | ARN of the ECS task execution role that workload.yml passes when registering a task definition revision. | `string` | n/a | yes |
| <a name="input_ecs_task_role_arn"></a> [ecs\_task\_role\_arn](#input\_ecs\_task\_role\_arn) | ARN of the ECS workload task role that workload.yml passes when registering a task definition revision. | `string` | n/a | yes |
...
| Name | Description |
| ---- | ----------- |
| <a name="output_role_arn"></a> [role\_arn](#output\_role\_arn) | ARN of the IAM role workload.yml assumes through GitHub OIDC. |
<!-- END_TF_DOCS -->
