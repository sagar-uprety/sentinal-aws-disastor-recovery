...
resource "aws_iam_role_policy" "runtime" {
  #checkov:skip=CKV_AWS_355:the ECS list/describe and RDS describe APIs needed for cross-region topology do not support complete resource scoping
...
resource "aws_security_group" "ecs" {
  ...
  #checkov:skip=CKV_AWS_382:the monitor needs HTTPS egress to the canonical workload and regional AWS APIs through NAT
  egress {
...
resource "aws_cloudwatch_log_group" "main" {
  #checkov:skip=CKV_AWS_158:a customer-managed Logs KMS key adds fixed cost to this portfolio monitor
