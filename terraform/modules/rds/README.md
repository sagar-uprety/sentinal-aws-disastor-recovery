...
PostgreSQL instance, or a cross-region read replica when `replicate_source_db_arn` is set, in isolated DB subnets reachable only from the ECS security group. Master password goes through `password_wo` (write-only) so plaintext never lands in Terraform state. Demo settings: `deletion_protection = false`, `skip_final_snapshot = true`.
...
| <a name="input_kms_key_id"></a> [kms\_key\_id](#input\_kms\_key\_id) | Destination-region KMS key ARN for replica storage. Required when replicate\_source\_db\_arn is set. | `string` | `null` | no |
| <a name="input_multi_az"></a> [multi\_az](#input\_multi\_az) | Enable Multi-AZ for high availability testing. | `bool` | `false` | no |
| <a name="input_password_wo"></a> [password\_wo](#input\_password\_wo) | Master password (write-only). Ignored for replicas, which inherit the source instance's credentials. | `string` | `""` | no |
...
| <a name="input_replicate_source_db_arn"></a> [replicate\_source\_db\_arn](#input\_replicate\_source\_db\_arn) | ARN of the source RDS instance; set to create a cross-region replica instead of a standalone instance. | `string` | `null` | no |
