...
CloudWatch alarms (ALB, RDS) plus an EventBridge rule for failed ECS deployments, routed to one SNS email subscription. Deliberately AWS-native rather than a separate observability ECS stack, so the monitoring path doesn't share a failure domain with the workload it watches.
...
| <a name="input_ecs_cluster_name"></a> [ecs\_cluster\_name](#input\_ecs\_cluster\_name) | ECS cluster name, used to derive the deployment-failure rule's default service ARN scope. | `string` | n/a | yes |
| <a name="input_ecs_desired_count"></a> [ecs\_desired\_count](#input\_ecs\_desired\_count) | Desired ECS task count; 0 relaxes the ALB healthy-host alarm for pilot-light standby. | `number` | `2` | no |
