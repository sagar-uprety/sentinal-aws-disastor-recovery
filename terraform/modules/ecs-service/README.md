...
Fargate cluster for the URL-shortener workload with ARM64 runtime, circuit breaker, and SSM-secured database and operator credentials.

## Design Intent

Tasks run in private application subnets behind the ALB security group. The task execution role receives narrowly scoped `ssm:GetParameters` on the regional database-password and link-token SecureStrings plus `kms:Decrypt` for the region-local KMS key. The workload task role has no AWS topology permissions. The service ships with a deployment circuit breaker (`enable = true`, `rollback = true`) using provider-supported defaults. `deploy_service = false` creates supporting resources before an image is available.
...
`kms:Decrypt` uses the region-local KMS key ARN prefix because the AWS-managed SSM key ARN cannot be known before parameter creation. This exception is documented per Hard Rule 7.
