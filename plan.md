...
- [~] `terraform/environments/monitoring` defines separate state, VPC, ECS, ALB, ECR, DNS, IAM, logs, alarms, SNS, security groups, DynamoDB, and outputs in eu-west-1. Deployment evidence now exists (applied live via `full-deploy`). Independent-state evidence (a plan/destroy run proving zero workload resources in this state) remains pending.
...
- [~] README, runbooks, cost text, least-privilege description, and M7 evidence template describe two-plane design. Live evidence remains pending for this specific write-up task (the underlying facts are now measured; the documents themselves have not yet been rewritten to match).
...
- [ ] Independent Terraform plans and destroy plans prove monitoring state has no workload resources and workload states have no monitoring resources. Not run this drill.
- [~] Local validation, CI, Terraform validation, TFLint, Checkov, Infracost, smoke tests, and container scans pass. `go build`/`vet`/`test`, ShellCheck, and CI (PRs #34, #35, #36) all passed; a dedicated Infracost/Checkov/TFLint re-run was not performed this session since no Terraform files changed.
