...
- `scripts/failback.sh`: guards and verifies the safe failback phases while leaving topology-changing Terraform edits manual. It creates and later deletes the temporary safety snapshot, verifies reverse-replica source and lag, requires the promoted prod database to be Multi-AZ and healthy with the known DR-written row before switching back, then verifies restored primary-to-DR replication and DR desired count 0. A destructive snapshot-based alternative must state its data-loss and RTO implications.
...
- [~] Historical M4 implementation created the Route53 hosted zone in `environments/dr/route53.tf` and delegated it through Cloudflare. Current configuration owns the zone in `environments/prod/route53.tf` so prod can validate the primary ACM certificate before DR depends on its remote state. M6 must delegate the current NS records, verify ACM DNS validation in both regions, and verify `https://status.sentinel.sagaruprety.com.np` through the ARC-gated record.
...
**Status 2026-07-20:** repository validation passed after a local PostgreSQL-backed test run. A later script review against current AWS guidance added confirmation gates, per-drill event isolation, verified ARC switching, row-based RPO, and failback reset checks; `bash -n scripts/*.sh` and ShellCheck through its official Docker image pass, and these hardened runtime paths remain unexecuted until M6. `terraform.yml` is syntax-checked and ready for its first GitHub Actions execution, and final-session documentation/templates are in place. No AWS workload resources were created. Bootstrap now declares the dedicated Terraform OIDC role; applying bootstrap, setting its ARN as a repository variable, GitHub environment approval, Infracost secret, workflow runs, deployments, drills, and teardown remain Milestone 6 work.

Tasks:
- [x] Deferred from M3: implement `terraform.yml` with fmt-check, validate, tflint, Checkov, Infracost diff, and PR-plan-comment jobs. Add separate approval-gated `workflow_dispatch` apply and destroy jobs. Merging code must not create infrastructure automatically.
...
Acceptance criteria:
- [x] A PR containing Terraform changes passes locally equivalent checks and has a ready-to-run workflow that will publish plan and Infracost information as a PR comment when exercised in GitHub Actions. Its apply and destroy paths require explicit workflow dispatch and environment approval.
...
Tasks:
- [ ] Apply bootstrap to create the shared GitHub OIDC provider and Terraform deployment role, set its output as `AWS_TERRAFORM_ROLE_ARN`. Apply prod Route53/ACM foundation, delegate the returned NS records from Cloudflare, wait for both regional ACM certificates to validate, then publish and verify an immutable image, apply prod service, then apply DR. Confirm HTTPS on both ALBs, HTTP-to-HTTPS redirect, ECR replication, plaintext-free state, required tags, canonical status URL target behavior, SSM parameter metadata, version compatibility, and pilot-light desired count before readiness declaration.
...
- [ ] Exercise the approved Terraform workflow in GitHub Actions: PR plan/Infracost comment, approval-gated deployment from zero, and a later code-only deployment with no manual AWS steps after approval.
- [ ] Complete the pending M4 topology-reset path after the first drill: create the temporary safety snapshot, rebuild prod as a replica of DR, run `failback.sh verify-replica`, promote and harden prod, run `failback.sh ready`, switch back with `switch-traffic.sh primary`, rebuild DR as prod's replica with desired count 0, run `failback.sh verify-reset`, then delete the temporary snapshot. Record reset duration and cost.
...
Acceptance criteria:
- [ ] A real PR with a Terraform change shows plan and Infracost information as a GitHub comment; an approved workflow deploys reviewed code from zero to healthy Sentinel tasks; a later code-only change deploys without manual AWS steps after approval.
...
Custom auth, user accounts, alerting rules inside the app, Kubernetes/EKS, multi-account setups, service mesh, AI/LLM features, any always-on infrastructure beyond the bootstrap state bucket.
