...
- Run the monitoring service as a separate always-on ECS service in eu-north-1 (separate from prod's eu-central-1 and dr's eu-west-1, so a regional incident in either drill region can't also take down the monitor) with its own VPC, ALB, security groups, persistence, ECR repository, Terraform root/state, and deployment workflow.
...
│   │   ├── acm-cert/         # regional ACM cert + DNS validation, used by prod and dr
│   │   ├── route53-failover/
│   │   └── ecr/
│   └── environments/
...
Tasks:
- [x] Modules: vpc, alb, workload-ecs-service, rds, ecr per section 4.1. Compose in environments/prod.
...
**Status 2026-08-07:** `terraform.yml`'s `full-deploy` operation had never been run to completion (added 2026-07-26, no code path had exercised it). Three real bugs were found and fixed live: the inline monitor/workload image builds used the wrong Docker build context, then built without `--platform linux/arm64` for Fargate Graviton, then lacked the QEMU setup needed to cross-build that platform at all. After those three fixes, `full-deploy` completed successfully end to end for the first time: monitoring foundation, monitor deploy, prod foundation, prod deploy, and DR pilot light all applied cleanly in one run. Live-verified via `curl` and `aws` CLI, not just "apply succeeded": monitor healthy at `https://sentinel.sagaruprety.com.np/healthz` (200) with `/topology` correctly returning real prod (2/2 tasks, Multi-AZ writer) and DR (0/0 pilot light, active replica, correct `read_replica_source`) state; workload healthy at `https://app.sentinel.sagaruprety.com.np/healthz` (200). Separately, a full UI/UX pass (own review plus an independent subagent review) fixed real bugs in the monitor and workload pages: DR pilot-light standby was being shown as "Degraded", the live-serving-region banner could contradict a region's own health badge, task IDs were truncated, several labels/colors were misleading or inconsistent, and all literal "Lego"/"Brick" branding was removed. The DynamoDB-backed drill-event feature (event storage, `/events` endpoint, timeline UI) was implemented in this same push and then deliberately removed by owner decision: `scripts/drill-lib.sh` keeps writing timestamped events to the local `DRILL_LOG` text file only, which is what `measure.sh` already read for RTO/RPO, so nothing measurement-relevant was lost. **Update 2026-08-08:** the UI/backend work above went through a conformance check against the AWS pilot-light whitepaper and an independent script-logic review before being deployed. That review found `measure.sh`'s RPO was structurally always 0 (it compared a value to itself instead of an independent DR-side observation), `failover.sh`/`failback.sh` couldn't resume after a crash mid-promotion, `simulate-ha.sh db` shared a weak confirmation gate with a trivial single-task stop, and the pre-outage link check only proved one write survived rather than every write. All four were fixed: `replica_lag_seconds` (real CloudWatch `ReplicaLag`) is now the sole RPO evidence, both promotion scripts detect and resume an in-flight promotion, `simulate-ha.sh db` requires its own `CONFIRM_HA_DB_FAILOVER`, and the survival check now verifies the full pre-outage link set. Merged via PR #34, then deployed live through `monitor.yml` and `workload.yml` directly (not `full-deploy`), the first time either workflow has been exercised as the actual deploy path. Confirmed live: `/status` on the real monitor now includes `sample_count_24h`, `/events` returns 404, no drill-timeline or old branding remains. Every UI/backend fix and script fix described above is now live and current.

**Update 2026-08-08 (live drill):** ran the full mandatory drill suite end to end on live infrastructure: three in-region HA tests, one full regional failover (with traffic switch), and one full failback (with topology reset), with the monitor observed continuously throughout. Before the regional drill, a conformance check against the AWS pilot-light whitepaper confirmed the architecture matches (dual RDS replication + backup replication, ARC as the data-plane traffic switch exactly matching AWS's own recommended pattern, manual-gated failover with automated steps). Results:
...
- Regional failover RTO: 666s (target 30 min). RPO: 26.0s, real CloudWatch `ReplicaLag`, not a placeholder (target 60s). Every link present on primary before the outage (not a sample of one) verified present in DR after promotion.
- Failback duration (DR writes frozen through primary traffic re-verified): 851s (14m11s), closely matching the historical M6 figure (840s).
...
One real infrastructure hiccup occurred mid-drill: a transient AWS CLI credential failure interrupted `failover.sh` immediately after the actual RDS promotion and DR verification work had already succeeded. Diagnosed live via `aws rds describe-events`, confirmed no data or state was lost, and manually completed the exact remaining verification steps the script itself would have run before continuing.

Two real bugs were found live and are now fixed and deployed (PRs #35, #36): the monitor's checker reused a pooled keep-alive HTTP connection across a DNS-driven region cutover. Go's default 90s idle-connection timeout never elapsed at a 30s check interval, so it kept reporting the pre-cutover region as down for minutes after traffic had genuinely moved (caught because the monitor's own status disagreed with a direct `curl` to the same URL). Separately, `measure.sh` read a `traffic_switch_requested` event that collides between the regional-failover switch and the later failback switch-back, producing a nonsense negative duration on a post-failback report (the RTO/RPO figures above were captured before this collision existed and are unaffected). Both are fixed and redeployed live. One cosmetic-only bug remains documented but doesn't block anything: `switch-traffic.sh`'s case-sensitivity check for its own next-step hint message was also fixed in the same pass.

One residual anomaly, not code-related: both DR's and prod's RDS instances, after being promoted and then converted to Multi-AZ mid-drill, reported `SecondaryAvailabilityZone: null` via the API despite `MultiAZ: true` and a clean "finished" event in AWS's own event log. Functionally harmless (the documented `MultiAZ` check that gates the runbook passed correctly both times) but worth a manual check in the AWS Console to confirm the standby is real.

Tasks:
- [x] URL-shortener code, tests, non-root container, local Compose support, and dedicated README exist. Live deployment evidence now exists: prod applied and serving real traffic at `app.sentinel.sagaruprety.com.np` (verified 200, 2/2 tasks), deployed through `workload.yml` itself (run succeeded end to end, its own dedicated publish/deploy path, not `full-deploy`'s inline build).
- [x] Monitor code targets only canonical workload health URL and reads explicit prod and DR ECS/RDS identifiers. Deployment and live topology evidence now exist: `/topology` on the real deployed monitor returns correct real prod and DR state (see status note above). Running image is current, deployed through `monitor.yml` on 2026-08-08.
- [x] Monitor-owned on-demand DynamoDB check persistence exists with no workload RDS connection. Drill-event persistence is local-only by final owner decision, not DynamoDB: scripts write timestamped events to `DRILL_LOG` (a plain-text file) and `measure.sh` reads only that file for RTO/RPO. There is no DynamoDB event storage, no monitor `/events` endpoint, and no drill-timeline UI; the checks-only DynamoDB table remains and is applied live.
- [x] `terraform/environments/monitoring` defines separate state, VPC, ECS, ALB, ECR, DNS, IAM, logs, alarms, SNS, security groups, DynamoDB, and outputs in eu-west-1. Deployment evidence now exists (applied live via `full-deploy`). Independent-state evidence completed 2026-08-14: after the 2026-08-08 combined destroy (run `31231322585`, see M8), all three workload/monitoring states are empty (`terraform state list` returns nothing for monitoring, prod, and dr). Fresh `terraform plan` against each empty state (61 creates for monitoring, 76 for prod) shows no cross-plane resource types in either plan, and DR's Terraform config has data-source dependencies only on prod (`aws_ecs_service.prod`, `aws_db_instance.prod`, `aws_lb.prod`, etc.), never on monitoring. `terraform plan -destroy` on all three empty states returns "No changes."
- [x] Prod and DR definitions run URL-shortener image while retaining replication, ARC, backups, alarms, and DR desired count semantics. Deployment evidence now exists: prod Multi-AZ writer + DR active replica applied and verified live. The ARC routing-control cluster itself is intentionally not provisioned outside a drill (matches design, not a gap).
- [x] DNS code assigns monitor apex and ARC-controlled workload subdomain. Live DNS and HTTPS evidence now exists: both hostnames verified 200 over HTTPS (see status note above).
...
Acceptance criteria:
- [x] Stopping, replacing, scaling, promoting, routing, resetting, or destroying workload resources does not restart, scale, reroute, or destroy monitoring resources. Verified empirically: the monitor process was never restarted, scaled, or interrupted by any HA, failover, or failback operation performed against workload resources during the 2026-08-08 drill.
- [x] Monitor remains publicly reachable and records canonical workload outage and recovery throughout ECS, RDS Multi-AZ, regional failover, and failback tests. The monitor's own health endpoint stayed reachable through every phase (confirmed by `monitor_available_during_outage`, `monitor_available_during_failover`, and `monitor_available_after_traffic_switch` events). Its recorded observations of the workload briefly lagged reality during DNS cutovers due to the checker connection-reuse bug found and fixed in this same drill; the monitor process itself was never down.
- [x] Monitor task role has only read-only ECS/RDS observation actions, documented wildcard resources where those APIs require them, access to its own DynamoDB table, and no ECS, RDS, ARC, Route53, or Terraform mutation permissions. Verified by direct inspection of `terraform/modules/monitor-ecs-service/main.tf`'s IAM policy: only `Describe*`/`List*`/`Get*` actions plus scoped `dynamodb:PutItem`/`Query` on its own table ARN.
...
- [x] A prod-created link survives failover, a DR-created link survives failback, and retained evidence records both checks alongside measured RTO/RPO. Confirmed live 2026-08-08: RTO 666s, RPO 26.0s (real `ReplicaLag`).
- [x] Independent Terraform plans and destroy plans prove monitoring state has no workload resources and workload states have no monitoring resources. Completed 2026-08-14 against live (post-teardown) state: all three states are empty; `terraform plan` for monitoring (61 creates) and prod (76 creates) contain no cross-plane resource types, and DR's data sources reference only prod. `terraform plan -destroy` on all three returns "No changes." See M7 tasks section above for full detail.
...
- [x] **DESTROY:** workload teardown, DR before prod. Requires explicit two-stage user confirmation. Bootstrap and isolated monitoring plane preserved unless separately approved for destruction. Executed 2026-08-08 via the reviewed `terraform.yml` `operation=destroy` dispatch, gated by the typed `confirm_destroy=DESTROY` input (run [31231322585](https://github.com/sagar-uprety/aws-pilotlight-multi-region-dr/actions/runs/31231322585), job "Destroy all environments (DR, prod, monitoring)", 19m51s, success). Per the workflow's fixed step order, DR was destroyed first, then prod, then monitoring (owner chose to also destroy monitoring here, beyond the DR-before-prod minimum). Confirmed live on 2026-08-14: no ECS clusters or RDS instances exist in either region, and all three Terraform states (monitoring, prod, dr) are empty. Bootstrap state (backend, OIDC roles, Route53 zone) is untouched.
...
- A fresh deploy of prod/dr/monitoring under the new names, a natural next action,, left for whenever the next drill is triggered via the existing per-environment `terraform.yml`/`monitor.yml`/`workload.yml` paths, not part of this rebrand's scope.
