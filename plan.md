...
Tasks:
- [~] Bootstrap applied locally on 2026-07-21: existing state backend retained; persistent Route53 zone, shared GitHub OIDC provider, Terraform deployment role, and read-only pull-request plan role created; drift-free follow-up plan confirmed; and both role ARNs configured in GitHub. Cloudflare delegation and both GitHub environments are configured. PRs #14–#17 verified prod speculative plans, merge-triggered saved-plan Terraform deployment, one-digest prod/DR application deployment, canonical apex migration, regional ACM ownership, HTTPS 200, `/healthz` 200, HTTP-to-HTTPS 301, generated-asset reproducibility, and stable single-leader target monitoring. PR #18 added the dedicated from-zero scheduler and the scoped IAM read permission required for Terraform role deletion; PR #20 added a typed manual trigger when the intended deployment date was found to be one day later than expected. Baseline teardown run `29873075087` then destroyed DR before prod successfully while preserving bootstrap. GitHub billing does not support required-reviewer environment rules for this repository; protected environments, typed confirmations, safety prechecks, and saved plans provide available gates. Manual run `29911585417` created healthy prod infrastructure and an immutable image but failed during the first DR plan because stale ACM import blocks referenced certificates deleted by the baseline teardown. Remaining: remove the stale imports, complete a from-zero DR deployment, prevent the superseded scheduled run from deploying again, and retain redacted evidence for plaintext-free state, required tags, database-password SSM metadata, version compatibility, and pilot-light desired count.
- [ ] Verify the applied prod and DR state snapshots contain no database-password plaintext; retain only redacted evidence of write-only SSM/RDS metadata and synchronized credential-version fields.
- [ ] Run final in-region HA demonstrations with the topology panel visible: stop one ECS task and observe a new or surviving task identity; stop all tasks observed in one AZ and confirm another-AZ service; force RDS Multi-AZ failover and record the writer/managed-standby AZ transition after recovery. Preserve the existing truthful limitation that task termination is not a complete AZ outage, and verify a known database row before and after forced database failover.
- [ ] Verify the website Recovery topology after every website-visible HA and DR transition. Retain before/during/after screenshots or exported `/topology` JSON showing serving task ID and AZ, ECS task IDs and AZ spread, active Region, and database writer/standby zones. Required checkpoints are ECS task replacement, controlled AZ-capacity loss and recovery, RDS Multi-AZ failover, DR traffic activation, prod traffic restoration, and final prod website health. An unavailable site during the injected outage is expected evidence; DR pilot-light desired count and replica direction after reset remain AWS/Terraform evidence because the active prod website cannot display standby control-plane state.
- [~] Exercise the infrastructure workflows in GitHub Actions. PRs #14–#17 verified PR quality checks, prod speculative-plan comments, merge-triggered exact-plan Terraform deployments, one-digest prod/DR application deployments, and a later code-only deployment with no manual AWS steps; main pushes now skip duplicate Terraform quality validation. PR #18 added a one-time fully automatic from-zero deployment scheduled for 2026-07-23 at 10:00 Europe/Berlin, and PR #20 added a typed manual trigger used for run `29911585417` on 2026-07-22. That run passed prod deployment and ECR replication, then failed at `Plan DR pilot light` because two ACM import blocks referenced certificates deleted by the baseline teardown. Remaining: remove the stale imports, rerun from the DR plan, remove or disable the superseded schedule before it fires, capture the first DR plan proving the stable RDS resource moves from replica configuration to standalone without database replacement, and exercise hardened guarded recovery plan/apply jobs. Retain local pre-commit Infracost output as cost-estimate evidence.
- [ ] Correct and verify DR pilot-light alarm semantics: healthy-host and running-task alarms remain `OK` while desired count is intentionally zero, change to active thresholds during failover, detect active-service loss, and return to standby thresholds during reset. Route53 detection checks use the dedicated HTTP `/healthz` forwarding rule rather than the redirect response, because Route53 treats an HTTP 3xx response as healthy without following it.
- [ ] Initialize and verify exactly one active ARC control before the drill, retain the atomic two-control data-plane update and exactly-one-active safety rules, and document that this showcase discovers identifiers through control-plane APIs immediately before switching. A production runbook would retain endpoints and control ARNs out of band.
...
- [ ] Harden and complete the topology-reset path after the mandatory drill. Before replacing former prod, snapshot the active DR writer and make `recovery.yml` verify snapshot source, freshness, and encryption. Freeze DR application writes, require fresh replication evidence, promote prod only while DR writes remain frozen, and verify the final observed DR row after promotion before traffic returns. Then rebuild DR as prod's replica with desired count zero, verify reset, and delete the temporary snapshot. Record reset duration and cost. Typed confirmations and exact saved plans are sufficient for this showcase; production should add independent post-plan approval. Historical context: M4 stopped after verifying the temporary reverse replica and then destroyed all resources at owner request, so this session starts fresh rather than resuming that topology.
- [ ] Run one mandatory full disaster drill: traffic on primary -> `CONFIRM_DISASTER=YES simulate-disaster.sh` -> independent outage review -> `CONFIRM_FAILOVER=YES failover.sh` -> DR readiness verification -> `CONFIRM_TRAFFIC_SWITCH=DR switch-traffic.sh dr` -> `measure.sh`. Report RTO through verified public DR traffic, operator-invocation duration, row-based observed RPO using the newest primary row observed while ECS drains, and fresh pre-promotion ReplicaLag evidence. Do not describe the observed row boundary as transactional proof of every final source commit.
- [ ] After the mandatory drill, execute and measure the topology reset. Completion requires replication, latest-known-row verification, desired counts, ARC state, and readiness checks to be restored.
- [ ] Query DR checks for the mandatory outage window and retain screenshots or exports alongside terminal, CloudWatch/SNS, and status-page evidence. Video evidence is not required.
...
- [~] Reduce DR coupling to prod remote state: DR-owned ACM lifecycle moved into DR state through PR #16, the apex certificate is attached, the deleted legacy hostname certificate is no longer declared, and stale one-time import blocks are removed. Remaining: resolve the Route53 zone and regional SSM parameter through explicit interfaces or AWS data sources; query primary RDS identity/version directly; configure DR instance class explicitly; and remove obsolete prod outputs.
...
- [ ] Destroy DR before prod after final evidence capture, unless live topology dictates a different dependency-safe order. Baseline run `29873075087` verified this order successfully on 2026-07-21 after the deployment role gained scoped `iam:ListInstanceProfilesForRole`; repeat after final M6 evidence and reconcile Terraform state/configuration.

Acceptance criteria:
- [ ] PR Terraform plan comments, local Infracost, and a later code-only deployment without manual AWS steps are verified through PRs #14–#17. Complete the remaining criterion when the approved M6 workflow deploys reviewed code from zero to healthy Sentinel tasks; manual run `29911585417` is retained failed-run evidence, not acceptance evidence.
- [ ] Applied prod and DR state contain no database-password plaintext; evidence shows only redacted write-only credential metadata and versions.
- [ ] The mandatory full drill meets or explicitly misses stated RTO/RPO targets with phase evidence. RTO ends at `traffic_verified`, after authoritative DNS and `/topology` prove eu-west-1 serves the canonical hostname. RPO uses the newest primary row observed during controlled ECS drain and the matching DR row, with fresh ReplicaLag retained as supporting evidence and the non-transactional observation limitation stated. The result is not omitted because it is worse.
- [ ] The topology reset is executed, verified, and measured after the mandatory drill. Primary-to-DR replica topology, single-writer operation, pilot-light desired counts, and ARC controls are restored.
...
- [ ] Website Recovery topology evidence matches each website-visible transition: task and AZ changes for HA, writer/standby AZ exchange for RDS failover, eu-west-1 while DR serves traffic, and eu-central-1 after failback. Standby-only state is verified separately through AWS/Terraform rather than inferred from the active website.
