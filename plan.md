...
│   └── terraform.yml         # quality, speculative plan/cost comments, and protected manual plan/apply/destroy
...
**Status 2026-07-20:** repository validation passed after a local PostgreSQL-backed test run. A later script review against current AWS guidance added confirmation gates, per-drill event isolation, verified ARC switching, row-based RPO, and failback reset checks; `bash -n scripts/*.sh` and ShellCheck through its official Docker image pass, and these hardened runtime paths remain unexecuted until M6. Both workflows now use descriptive jobs and steps, job-scoped OIDC permissions, protected environments, serialized production operations, saved Terraform plans, immutable image digests, and post-deployment verification. No AWS workload resources were created. Applying bootstrap, setting role ARN variables, GitHub environment approval, Infracost secret, workflow runs, deployments, drills, and teardown remain Milestone 6 work.

Tasks:
- [x] Deferred from M3: implement `terraform.yml` with format, validation, TFLint, Checkov, Infracost, and explicit prod-only speculative PR-plan jobs. Manual deploy and destroy use protected environments, serialized execution, dependency-safe ordering, saved plans, immutable image digests, and post-deployment checks. Merging code must not create infrastructure automatically.
...
Tasks:
- [ ] Apply bootstrap locally once to create the shared GitHub OIDC provider and Terraform deployment role, then set its output as `AWS_TERRAFORM_ROLE_ARN`. Configure protected environments `terraform-production` and `production`. Dispatch `operation=prepare-dns`, delegate the emitted Route53 NS records through Cloudflare, then dispatch `operation=deploy`; after prod foundation creates the app role, set `AWS_ROLE_ARN` before the later code-only application deployment. Verify both regional ACM certificates, HTTPS/redirect behavior, ECR replication, plaintext-free state, required tags, canonical target behavior, SSM metadata, version compatibility, and pilot-light desired count.
