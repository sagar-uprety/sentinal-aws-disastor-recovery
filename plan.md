...
- [~] Historical M4 implementation created the Route53 hosted zone in `environments/dr/route53.tf` and delegated it through Cloudflare. Current configuration owns the persistent zone in bootstrap so nameservers remain stable across workload deploy/destroy cycles; prod discovers it by name for ACM validation and DR receives its ID through prod remote state. M6 must verify ACM DNS validation in both regions and verify the dashboard at `https://sentinel.sagaruprety.com.np` plus machine health at `/healthz` through the ARC-gated apex record. The 2026-07-17 rehearsal evidence retains the historical `status.sentinel.sagaruprety.com.np` hostname used during that run.
...
Tasks:
- [~] Bootstrap applied locally on 2026-07-21: existing state backend retained; persistent Route53 zone, shared GitHub OIDC provider, Terraform deployment role, and read-only pull-request plan role created; drift-free follow-up plan confirmed; and both role ARNs configured in GitHub. The PR role uses `ReadOnlyAccess`, read-only state access, and exact `pull_request` OIDC trust so speculative plans do not enter the main-only production environment. Cloudflare delegation and both GitHub environments are configured; PR #14 verified the prod speculative-plan comment, merge-triggered saved-plan Terraform deployment, and one-digest prod/DR application deployment. GitHub billing does not support required-reviewer environment rules for this repository; protected environments, typed confirmations, safety prechecks, and saved plans provide available gates. Remaining: apply and verify the canonical apex-hostname migration, exercise a reviewed deployment from zero, and retain evidence for both regional ACM certificates, HTTPS/redirect behavior, ECR replication, plaintext-free state, required tags, canonical target behavior, database-password SSM metadata, version compatibility, and pilot-light desired count.
...
- [~] Exercise the infrastructure workflows in GitHub Actions. PR #14 verified quality checks, prod speculative-plan comment, merge-triggered saved-plan deployment, and one-digest prod/DR application deployment. Remaining: explicitly dispatched deployment from zero, first DR plan proving `aws_db_instance.replica[0]` moves to `aws_db_instance.main[0]` without database replacement, guarded recovery plan/apply jobs, and a later code-only deployment with no manual AWS steps. Retain local pre-commit Infracost output as cost-estimate evidence.
