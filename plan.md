...
- [~] `monitor.yml` and `workload.yml` independently publish and deploy images. Workflow execution evidence remains pending; the live deployment so far went through `full-deploy`'s own inline build, not these workflows.
