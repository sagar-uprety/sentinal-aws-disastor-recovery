...
- [~] Exercise the infrastructure workflows in GitHub Actions. PRs #14–#17 verified PR quality checks, prod speculative-plan comments, merge-triggered exact-plan Terraform deployments, one-digest prod/DR application deployments, and a later code-only deployment with no manual AWS steps; main pushes now skip duplicate Terraform quality validation. PR #18 added a one-time fully automatic from-zero deployment scheduled for 2026-07-23 at 10:00 Europe/Berlin. Remaining: verify that scheduled run, capture the first DR plan proving `aws_db_instance.replica[0]` moves to `aws_db_instance.main[0]` without database replacement, and exercise guarded recovery plan/apply jobs. Retain local pre-commit Infracost output as cost-estimate evidence.
- [ ] Correct and verify DR pilot-light alarm semantics: healthy-host and running-task alarms remain `OK` while desired count is intentionally zero, change to active thresholds during failover, detect active-service loss, and return to standby thresholds during reset.
...
- [ ] Complete the topology-reset path after the first drill: create the temporary safety snapshot; dispatch `recovery.yml` with `operation=failback-prepare` and its typed confirmation; run `failback.sh verify-replica`, `promote-primary`, and `ready`; switch back with `switch-traffic.sh primary`; dispatch `recovery.yml` with `operation=failback-reset` and its typed confirmation; run `failback.sh verify-reset`; then delete the temporary snapshot. Record reset duration and cost. Historical context: M4 stopped after verifying the temporary reverse replica and then destroyed all resources at owner request, so this session starts fresh rather than resuming that topology.
...
- [~] Reduce DR coupling to prod remote state: DR-owned ACM lifecycle moved into DR state through PR #16 and the apex certificate is attached. Remaining: resolve the Route53 zone and regional SSM parameter through explicit interfaces or AWS data sources; query primary RDS identity/version directly; configure DR instance class explicitly; remove obsolete prod outputs; and delete the now-detached legacy `status.sentinel.sagaruprety.com.np` DR certificate.
...
Acceptance criteria:
- [ ] PR Terraform plan comments, local Infracost, and a later code-only deployment without manual AWS steps are verified through PRs #14–#17. Complete the remaining criterion when PR #18's scheduled workflow deploys reviewed code from zero to healthy Sentinel tasks.
