...
│   └── terraform.yml         # quality, speculative plan/cost comments, and protected manual deploy/destroy
...
Tasks:
- [ ] Apply bootstrap locally once to create the state backend, persistent Route53 zone, shared GitHub OIDC provider, and Terraform deployment role. Delegate bootstrap's emitted NS records through Cloudflare, set the role output as `AWS_TERRAFORM_ROLE_ARN`, and configure protected environments `terraform-production` and `production`. Dispatch `operation=deploy`; after prod foundation creates the app role, set `AWS_ROLE_ARN` before the later code-only application deployment. Verify both regional ACM certificates, HTTPS/redirect behavior, ECR replication, plaintext-free state, required tags, canonical target behavior, SSM metadata, version compatibility, and pilot-light desired count.
...
- [ ] Exercise the approved infrastructure workflows in GitHub Actions: PR plan/Infracost comment, approval-gated deployment from zero, first DR plan proving `aws_db_instance.replica[0]` moves to `aws_db_instance.main[0]` without database replacement, guarded recovery plan/apply approvals, and a later code-only deployment with no manual AWS steps after approval.
...
- [ ] Complete the pending M4 topology-reset path after the first drill: create the temporary safety snapshot; dispatch and approve `recovery.yml` with `operation=failback-prepare`; run `failback.sh verify-replica`, `promote-primary`, and `ready`; switch back with `switch-traffic.sh primary`; dispatch and approve `recovery.yml` with `operation=failback-reset`; run `failback.sh verify-reset`; then delete the temporary snapshot. Record reset duration and cost.
